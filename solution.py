...
import os
from common.utils import fit, model_size
...
def Run_Accelergy(path_to_eyeriss_files='Q3'):
    ...
    print("path_to_eyeriss_files", path_to_eyeriss_files)
    ...
    for name, m in model.named_modules():
        if isinstance(m, nn.Conv2d):
            if hasattr(m, 'weight_mask'):
                ...
                ##### WRITE CODE HERE #####
                num_zero_channels = int(torch.sum(m.weight_mask == 0))
            else:
                num_zero_channels = 0
            ...
            stride = m.stride[0]
            height = input_activation[name].size(2)
            width = input_activation[name].size(3)
            print(f" {str(in_channels)} {str(out_channels)} {str(kernel_size)} {str(stride)} {str(height)} {str(width)}")
            conv_layer_generator(base_path, in_channels, out_channels, kernel_size, stride, height, width,  path)
